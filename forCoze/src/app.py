...
def update_waiting_time():
    """更新等待时间的线程函数"""
    global evaluation_in_progress, start_time

    while evaluation_in_progress:
        current_time = time.time()
        elapsed_time = current_time - start_time
        yield f"⏳ 评估进行中... 已等待 {elapsed_time:.1f} 秒\n\n请稍候，系统正在分析您的项目..."
...
def evaluate_with_coze(project_name, project_description, project_field, student_level, submission_materials):
    ...
    def add_status(status):
        status_updates.append(f"⏰ {current_time} | {status}")
        return "\n".join(status_updates)

    initial_status = add_status("🔄 开始评估流程...")

    if not coze or not workflow_id:
        status = add_status("⚠️ 使用模拟模式 (Coze配置未完成)")
        result = evaluate_mock(project_name, project_description, project_field, student_level, submission_materials)
        end_time = time.time()
        processing_time = end_time - start_time
        status = add_status(f"✅ 评估完成 (耗时: {processing_time:.2f}秒)")

        final_output = f"## 📊 检索状态与时间\n{status}\n\n---\n\n{result}"
        return final_output

    try:
        status = add_status("🔗 连接到Coze工作流...")
        ...
        status = add_status(f"📋 处理完成，共接收 {event_count} 个事件...")
        ...
        end_time = time.time()
        processing_time = end_time - start_time
        status = add_status(f"✅ 评估完成 (总耗时: {processing_time:.2f}秒)")
        ...
        end_time = time.time()
        processing_time = end_time - start_time
        status = add_status(f"❌ 发生错误: {str(e)}")
        status = add_status(f"⚠️ 切换到模拟模式 (耗时: {processing_time:.2f}秒)")
...
def evaluate_project(project_name, project_description, project_field, student_level, submission_materials):
    ...
    try:
        result = evaluate_with_coze(project_name, project_description, project_field, student_level,
...
def create_interface():
    ...
    with gr.Blocks(theme=gr.themes.Soft(), title="学生项目智能评价系统") as demo:
        ...
        # 定义按钮点击时的JavaScript行为
        demo.load(js="""
        function() {
            // 存储原始按钮文本
            window.originalButtonText = "开始评估";
        }
        """)

        # 绑定事件
        evaluate_btn.click(
            fn=lambda: gr.update(interactive=False, value="评估中..."),
            ...
            fn=lambda: gr.update(interactive=True, value="开始评估"),
            outputs=evaluate_btn
        )
